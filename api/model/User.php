...
class User
{
	...
	public function is_connected($rank){
		...
		{
			global $db;
			$query= $db->prepare('SELECT id,name,pwd,rank FROM user WHERE id=:id');
	...
	public function registeremail(){
		...
		$query=$db->prepare('INSERT INTO user (rank,name,pwd,email,keyactive,registerdate,cptwarnings,photo) VALUES (1,:name, :pwd, :email, :keyactive, :registerdate,0,:photo)');
		...
		$message="<html><body>"."\r\n";
		$message.="<h1 style='font-family:sans-serif;font-size:20px;color:#161616;'>Confirme ton inscription The Challenger</h1>"."\r\n";
		...
		$message.="<p style='font-size:14px;font-family:sans-serif;padding:20px 0;'><a style='color:#fff; font-weight:bold;padding:10px; background-color:#437B1A;border:1px solid #366B11;text-decoration:none;'href='http://www.yorka-design.fr/thechallenger/api/user/confirm/".$key."'>ACTIVER MON COMPTE THE CHALLENGER</a></p>"."\r\n";
		$message.="<p style='font-size:13px;font-family:sans-serif'>Ou copie/colle ce lien dans ton navigateur internet:</p><p style='font-size:13px;font-family:sans-serif'>http://www.yorka-design.fr/thechallenger/api/user/confirm/".$key."</p>"."\r\n";
	...
	public function checkfollow($id){
		global $db;
		$query=$db->prepare('SELECT id FROM follow WHERE idfollower=:idfollower AND idfollowed=:idfollowed');
	...
	public function checkWarnings(){
		global $db;
		$query=$db->prepare('SELECT cptwarnings FROM user WHERE id = :id');		
		$query->bindParam(':id', $_COOKIE['id'],PDO::PARAM_INT);
		$query->execute();
		$datas=$query->fetch();
		$query->CloseCursor();
